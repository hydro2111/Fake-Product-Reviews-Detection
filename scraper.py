    ...
    try:
        ...
        for review_block in soup.select('.review'):
            ...
            if 'Read more' in review_text:
                full_review_element = review_block.select_one('.full-review')
                if full_review_element:
                    review_text = full_review_element.get_text(strip=True)
            
            # Explicitly remove "Read more" in case it was still present
